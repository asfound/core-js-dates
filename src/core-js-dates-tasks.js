...
function getWeekNumberByDate(date) {
  ...
  const daysSinceStart = (date - yearStart) / MS_PER_DAY;

  const weekNumber = Math.ceil(daysSinceStart / DAYS_PER_WEEK);

  const isSameDay = yearStart.getDay() === date.getDay();

  return isSameDay ? weekNumber + 1 : weekNumber;
}
...
function getNextFridayThe13th(date) {
  ...
  for (;;) {
    const guessDate = new Date(currentDate.year, currentDate.month, 13);
    if (guessDate.getDay() === 5) {
      return guessDate;
    }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const periodInDays = getCountDaysOnPeriod(
...
function isLeapYear(date) {
  const daysCount = new Date(date.getFullYear(), 2, 0).getDate();
  return daysCount === 29;
}
